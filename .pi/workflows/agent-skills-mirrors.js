...
const canonicalWorktree = (value) => typeof value === "string" && value.startsWith("/private/tmp/") ? value.slice(8) : value;
const sameWorktree = (left, right) => safeWorktree(left) && safeWorktree(right) && canonicalWorktree(left) === canonicalWorktree(right);
...
const cleanupCandidate = async (path, label) => {
  ...
  if (!exact(verification,["registered","clean","path","message"]) || verification.registered !== true || verification.clean !== true || !sameWorktree(verification.path,path) || typeof verification.message !== "string") return { cleaned:false,message:"Worktree registration and cleanliness could not be proven." };
  ...
  if (!exact(removal,["cleaned","path","message"]) || removal.cleaned !== true || !sameWorktree(removal.path,path) || typeof removal.message !== "string") return { cleaned:false,message:"Ordinary worktree removal was not confirmed." };
...
const validReview = (review, current) => exact(review,["approved","message","findings","fixRequests","candidate_worktree","base_commit","candidate_commit"]) && typeof review.approved === "boolean" && boundedText(review.message) && boundedFindings(review.findings) && validFixRequests(review.fixRequests) && (!review.approved || review.fixRequests.length === 0) && sameWorktree(review.candidate_worktree,current.data.candidate_worktree) && review.base_commit === current.data.base_commit && review.candidate_commit === current.data.candidate_commit;
const sameBaseline = (left, right) => sameWorktree(left.candidate_worktree,right.candidate_worktree) && left.base_commit === right.base_commit && left.primary_head === right.primary_head && left.origin_main === right.origin_main && left.primary_clean === right.primary_clean;
...
const rawFinalizer = await agent(`${common}\nThe candidate was approved in this same reviewer session. Finalize only exact reviewed worktree ${current.data.candidate_worktree}, base=${current.data.base_commit}, candidate=${current.data.candidate_commit}. Do not modify candidate content. Reverify exact HEAD and primary still clean at origin/main. On failure do not push/deploy. On success push exactly the reviewed commit to origin/main without force; fast-forward the local primary checkout and run apply/doctor locally; then SSH only to macbook, require clean and fast-forwardable, fast-forward it, and run apply/doctor there. On each host, doctor must prove all managed runtime roots (~/.agents/skills and ~/.claude/skills) are healthy. Return status=complete and deployment.macmini/macbook=applied-and-verified only when repo sync, apply, and doctor all succeeded on both hosts; otherwise return blocked with each unverified host as not-run and a specific human action. Never reset/stash/force or SSH to macmini. Do not remove the candidate worktree. Preserve candidate_worktree, base_commit, and candidate_commit identity; report primary_head and origin_main from post-finalization command evidence. A complete result requires both to equal the exact candidate_commit and primary_clean=true.`, {label:"mirror-review-finalize",subagent_type:"daily-driver",session_key:reviewerSession,schema:envelopeSchema});
const finalizer = normalizeEnvelope(rawFinalizer);
const finalizerOutcomeValid = finalizer && (finalizer.status === "complete"
  ? finalizer.data.deployment.committed === true && finalizer.data.deployment.pushed === true && finalizer.data.deployment.macmini === "applied-and-verified" && finalizer.data.deployment.macbook === "applied-and-verified" && finalizer.data.human_actions.length === 0 && finalizer.data.primary_head === current.data.candidate_commit && finalizer.data.origin_main === current.data.candidate_commit && finalizer.data.primary_clean === true
  : finalizer.data.human_actions.length > 0);
const finalizerIdentityMatches = finalizerOutcomeValid && sameWorktree(finalizer.data.candidate_worktree,current.data.candidate_worktree) && finalizer.data.base_commit === current.data.base_commit && finalizer.data.candidate_commit === current.data.candidate_commit;
