...
5. Run repository validation and classify freshness by tree hash, not source commit alone. In workflow audit results, `pending_updates` means an upstream skill tree differs from the locked local tree and awaits live review; `metadata_updates` means source/ref/lock commit movement with an unchanged mirrored tree; `rejected_updates` is reserved for candidates that failed policy, security, dependency, or reviewer approval. Update the decision cache with supported source commits/tree hashes; do not invent facts.
