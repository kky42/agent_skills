...
``data/kernels.db`` and ``data/notebooks``. Keep merged helper state under the
same project-local cache root as the rest of this skill so generated files stay
out of git.
"""
...
def default_db_path(root: Path | None = None) -> Path:
    return cache_root(root) / "nvidia" / "kernels.db"


def default_notebook_cache_dir(root: Path | None = None) -> Path:
    return cache_root(root) / "nvidia" / "notebooks"
