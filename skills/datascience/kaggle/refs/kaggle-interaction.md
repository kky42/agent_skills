...
using your logged-in Chrome session via OpenCLI browser. No cookies, no
CSRF tokens — your browser session handles auth.
...
Before any browser-based operation, verify OpenCLI is connected:
...
opencli browser open "https://www.kaggle.com/competitions/{SLUG}/discussion"

# 2. Inspect to find the "New Topic" button
opencli browser state

# 3. Click "New Topic" button (ref varies per render — find it first)
opencli browser click <new-topic-ref>

# 4. Wait for the form to appear
opencli browser wait time 2

# 5. Fill in the title
opencli browser type <title-input-ref> "Your topic title"

# 6. Fill in the body (markdown supported)
opencli browser type <body-textarea-ref> "Your content here..."

# 7. Click "Publish Topic"
opencli browser click <publish-ref>
...
- Numeric refs change between sessions — always run `opencli browser state`
...
opencli browser open "https://www.kaggle.com/competitions/{SLUG}/discussion/{TOPIC_ID}"
...
opencli browser type <reply-textarea-ref> "Your reply here..."

# 6. Click the "Post Comment" button
opencli browser click <post-ref>
...
opencli browser open "https://www.kaggle.com/code/{OWNER}/{NOTEBOOK_SLUG}"
...
# 4. Type the comment
opencli browser type <comment-textarea-ref> "Your comment here..."
...
1. Navigate to the relevant page with `opencli browser open`
2. Use `opencli browser state` to discover interactive elements
...
```bash
opencli browser find --css "button" --limit 10
opencli browser find --css "textarea"
opencli browser find --css "input[placeholder*='Topic']"
```
